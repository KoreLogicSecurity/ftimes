...
######################################################################
#
# $Id: ftimes-crv2dbi.pl,v 1.24 2014/07/18 06:40:44 mavrik Exp $
#
######################################################################
#
...
######################################################################
#
# Purpose: Preprocess FTimes carve data for MySQL DB import.
#
######################################################################
...
######################################################################
  ...
  (
    ...
    'unit_size'     => "int unsigned not null, index unit_size_index (unit_size)",
    'range_list'    => "blob not null",
  );
  ...
  ####################################################################
  #
  # A Hostname, '-h', is optional. It affects the primary index, but
  # remember that primary index (as used here) is different than the
  # primary key. We can't guarantee that the primary index will be
  # unique across all records since more than one carve range could
  # start at the same offset in the same file. This is unlikely, but
  # it might happen. For example, the block at a given offset could
  # legitimately be the start of more than one file.
  #
  ####################################################################
  ...
  {
    $sPrimaryIndex = "index primary_index (name(255), offset)";
  }
...
ftimes-crv2dbi.pl - Preprocess FTimes carve data for MySQL DB import
...
This utility takes FTimes carve data as input, processes it, and
...
MySQL -- dbi is short for DB Import.
...
directly into the .sql file. The default value is 'carve'.
