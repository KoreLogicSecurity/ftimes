...
######################################################################
#
# $Id: ftimes-dig2dbi.pl,v 1.4 2007/02/23 00:22:36 mavrik Exp $
#
######################################################################
#
...
######################################################################
  ...
  # unique across all records since more than one dig string could
  # have a match in the same file at the same offset.
  #
  ####################################################################
  ...
  my $sTable = (exists $hOptions{'t'}) ? $hOptions{'t'} : "dig";
...
B<ftimes-dig2dbi.pl> B<[-F]> B<[-d db]> B<[-h host]> B<[-m max-rows]> B<[-t table]> B<-f {file|-}>
