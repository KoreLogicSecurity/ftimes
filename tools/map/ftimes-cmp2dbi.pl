...
######################################################################
#
# $Id: ftimes-cmp2dbi.pl,v 1.31 2014/07/18 06:40:45 mavrik Exp $
#
######################################################################
#
...
######################################################################
  ...
  (
    ...
    'records'       => "varbinary(24) not null", # length("-2147483648,-2147483648") + 1
...
B<ftimes-cmp2dbi.pl> B<[-D {default|ignore|replace}]> B<[-d db]> B<[-h host]> B<[-m max-rows]> B<[-o option[,option[,...]]]> B<[-t table]> B<-f {file|-}>
