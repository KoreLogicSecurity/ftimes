...
######################################################################
#
# $Id: nph-ftimes.cgi,v 1.37 2014/07/18 06:40:45 mavrik Exp $
#
######################################################################
#
...
######################################################################
  ...
  $hProperties{'Version'} = sprintf("%s %s", __FILE__, ('$Revision: 1.37 $' =~ /^.Revision: ([\d.]+)/));
...
sub ProcessGetRequest
{
  ...
  if ($sQueryString =~ /$$phProperties{'GETRegex'}/)
  {
    ...
    $$phProperties{'ClientRequest'} =~ /^(Map|MapFull|MapLean|Dig|DigFull|DigLean)Config$/;
