...
the following command line:
    ...
    ftimes --mapfull map.cfg

where map.cfg contained the following directives:
...
(see below) to an existing account on an Integrity Server.
...
B<URLPutURL>, B<URLAuthType>, B<URLUsername>, and B<URLPassword>,
the controls in the following config file may be obtained or derived
by reviewing the log file.
...
The following command will attempt to download a map config file
from the specified Integrity Server.  If successful, the contents
of the config file will be written to stdout.

    ftimes --getmode example5.cfg

If you want to download directly to a file, you can, of course,
redirect the output.  Another approach would be to add the
B<OutFileName> control to your config file.  Then, if you wanted
to download a config file and take a snapshot in one operation,
simply enable B<GetAndExec>.  This causes B<FTimes> to restart
in map mode (i.e. B<mapfull>).  If B<URLGetRequest> was set to
DigConfig, then B<FTimes> would request a dig config file and
subsequently restart in dig mode (i.e. B<digfull>).
