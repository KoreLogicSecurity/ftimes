...
=item BaseName_YYYYMMDDHHMMSS.cfg

The upload config file.  This file is created automatically when
B<URLCreateConfig> is enabled, and may be used in conjunction with
put mode to upload a previously created snapshot.

=item BaseName_YYYYMMDDHHMMSS.log
...
=item BaseName_YYYYMMDDHHMMSS.out
...
control, (2) /usr/local/integrity/etc/xmagic or c:\integrity\etc\xmagic,
