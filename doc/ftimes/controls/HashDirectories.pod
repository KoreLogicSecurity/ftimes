...
Applies to B<mapfull>.

B<HashDirectories> is optional.  When enabled, (i.e. 'Y') it
...
following hashes:
  ...
  Hash(D2) = MD5(MD5(F2))
  Hash(D1) = MD5(MD5(F1), Hash(D2))

If an entry within the directory is a special file (e.g., a device)
or cannot be opened/hashed, a 16 byte string consisting of all
zeros is used for the computation.
