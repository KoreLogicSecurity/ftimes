...
=item B<ExcludeFilterMd5>: <regexp>

Applies to B<mad>, and B<map>.

B<ExcludeFilterMd5> controls are optional, and there is no predefined
limit on the number that may be specified.  However, there can be only
one B<ExcludeFilterMd5> control/value pair per line.

Each B<ExcludeFilterMd5> value must be specified as a Perl compatible
...
Any record containing an MD5 that is matched by an B<ExcludeFilterMd5>
...
=item B<ExcludeFilterSha1>: <regexp>

Applies to B<mad>, and B<map>.

B<ExcludeFilterSha1> controls are optional, and there is no predefined
limit on the number that may be specified.  However, there can be only
one B<ExcludeFilterSha1> control/value pair per line.

Each B<ExcludeFilterSha1> value must be specified as a Perl compatible
regular expression with the exception that capturing '()' subpatterns
and back references are not allowed.  You can use '(?:)' if you
require parentheses for grouping purposes.

Any record containing an SHA1 that is matched by an B<ExcludeFilterSha1>
value will be rejected.  All other records will be written to the
output stream subject to the rules of include filter processing.  If
no filters are specified, exclude filter processing is implicitly
disabled.
...
=item B<IncludeFilterMd5>: <regexp>

Applies to B<mad>, and B<map>.

B<IncludeFilterMd5> controls are optional, and there is no predefined
limit on the number that may be specified.  However, there can be only
one B<IncludeFilterMd5> control/value pair per line.

Each B<IncludeFilterMd5> value must be specified as a Perl compatible
regular expression with the exception that capturing '()' subpatterns
and back references are not allowed.  You can use '(?:)' if you
require parentheses for grouping purposes.

Any record containing an MD5 that is matched by an B<IncludeFilterMd5>
...
=item B<IncludeFilterSha1>: <regexp>

Applies to B<mad>, and B<map>.

B<IncludeFilterSha1> controls are optional, and there is no predefined
limit on the number that may be specified.  However, there can be only
one B<IncludeFilterSha1> control/value pair per line.

Each B<IncludeFilterSha1> value must be specified as a Perl compatible
regular expression with the exception that capturing '()' subpatterns
and back references are not allowed.  You can use '(?:)' if you
require parentheses for grouping purposes.

Any record containing an SHA1 that is matched by an B<IncludeFilterSha1>
value will be written to the output stream.  All other records will be
rejected.  If no filters are specified, include filter processing is
implicitly disabled.
